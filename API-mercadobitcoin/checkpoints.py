...
class dataingestor(ABC):
    ...
    def __init__(self,writer:datawriter,coins: List[str], default_start_date: datetime.date) -> None:
        ...
        self.writer = writer
        self._checkpoint = self._load_checkpoint()  # _ no inicio indica que é um atributo interno não acessado fora da classe
    ...
    def _write_checkpoint(self):
        with open(self._checkpoint_filename,"w") as f:  # w é o método write que vai sobrescrever
    ...
    def _load_checkpoint(self) -> datetime:
        try:
            ...
            return None  #quando roda a primeira vez se não tiver o arquivo recebe None

    def _get_checkpoint(self): 
        #se o checkpoint ainda é None ele vai recebe a data default, se não ele é igual ao checkpoint
...
class daysummaryingestor(dataingestor):
    ...
    def ingest(self) -> None:
        ...
        if date< datetime.date.today():
            ...
            self._update_checkpoint(value= date + datetime.timedelta(days=1))


#%%
#criando um job
ingestor = daysummaryingestor(writer=datawriter,coins=['BTC', 'ETH','LTC'],default_start_date=datetime.date(2022,8,1))
